...
def tokenize_batch(examples, tokenizer):
    # Ensure EOS token separates documents
    outputs = tokenizer(examples["text"])
    # Manually append EOS token ID to input_ids and attention_mask
    eos_id = tokenizer.eos_token_id
    for i in range(len(outputs["input_ids"])):
        outputs["input_ids"][i].append(eos_id)
...
def main(args):
    ...
    log("=== Pre-training setup ===", filepath=args.log_path)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--dataset_config", type=str, default="CC-MAIN-2024-10",
                    help="Dataset config (e.g., wikitext-103-raw-v1).")
    ...
    args.output_dir = f'./results/pretrain_{args.model_name}_{"-".join(args.dataset_name.split("/"))}_lr-{args.lr}_token-{args.train_tokens}_disp-{args.dispersion}-{args.dispersion_coeff}-{args.dispersion_loc}_fewshot-{args.num_fewshot}_maxsample-{args.max_eval_samples}_seed-{args.seed}'
